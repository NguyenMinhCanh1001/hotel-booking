...
if (isset($_POST['add_room'])) 
{
    ...
    if($stmt = mysqli_prepare($con,$q2)){
       ...
       $flag = 0; 
       die('Lỗi khi chuẩn bị truy vấn chèn dữ liệu');
    }
    ...
    while($row = mysqli_fetch_assoc($res))
    {
        ...
        $data .="
            ...
            <td>{$row['quantity']}</td>
                <td>$status</td>
                <td>
                    <button type='button' onclick='edit_details($row[id])' class='btn btn-dark shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#edit_room'>
                       ...
                       <button type='button' onclick='romve_room($row[id])' class='btn btn-danger shadow-none btn-sm'>
...
if (isset($_POST['get_room_images'])) {
    ...
    while($row = mysqli_fetch_assoc($res)){
        if($row['thumb'] == 1){
            $thumb_btn = "<i class='bi bi-check2 text-light bg-success px-2 py-1 rounded fs-5 btn-sm'></i>";  
        }
        else
        {
